#ifndef AmericanOptionSolver_HPP
#define AmericanOptionSolver_HPP
...
public:
	AmericanOptionSolver(bool isCall_,double exactP_, double K_, double sigma_, double s0_, double q_, double T_, double r_, int M_, double alpha_, double tol_, double w_) :
		isCall(isCall_),exactP(exactP_),M(M_), K(K_), sigma(sigma_), s0(s0_), q(q_), T(T_), r(r_), alpha(alpha_), tol(tol_), w(w_) {
		...
		//std::cout << "N=" << N << "\n";
		//std::cout << "Exact=" << (xright - xleft) / std::sqrt(dt / alpha) << "\n";
	...
	std::tuple<double, double> ApproximateVarRed(bool useFE) {
		EuroOptionSolver euroSolver(isCall, K, sigma, s0, q, T, r, M, alpha, tol, w);
